...
utilising a specific $k$ number of active antennas, from a total number of available 
physical $n$ pads, the number of unique combinations $C$ is formalised as
\begin{equation}
...
astronomical literature. Prior work addressed the antenna placement problem using continuous optimisation 
...
early in the search process. The acceptance of inferior solutions provides a mechanism 
to escape local optima~\cite{engelbrecht2007}. Comparison of the population-based genetic algorithm against the trajectory-based 
simulated annealing provides an evaluation of search strategies for discrete aperture synthesis 
optimisation.

\subsubsection{Genetic Algorithms}
A GA is a population-based search technique inspired by Darwinian evolution~\cite{engelbrecht2007}. In the context of 
array design, the GA maintains a diverse pool of candidate layouts. A diverse pool allows fit genetic traits, 
specifically high-performing antenna pairings, to propagate through successive generations via 
recombination~\cite{bremermann1962}. To prevent premature convergence on local optima, a fitness 
sharing mechanism is utilised to penalise candidate solutions that are spatially similar. This 
enforces population diversity. Furthermore, an elitism mechanism guarantees the survival of the 
highest-performing layout across generations. The procedural flow of the GA is outlined in Algorithm 1.
...
SA is a trajectory-based metaheuristic modelled after 
...
\begin{enumerate}
    \item \textbf{Standard Baseline:} The baseline configuration utilised a tournament size of five and a mutation rate 0.15.
     The parameters included a moderate tournament size and a standard mutation rate. The baseline configuration established an evolutionary performance benchmark.
    \item \textbf{Reduced Selection Pressure:} The tournament size was reduced from five to two. The reduction was designed to decrease selection pressure. Decreased selection pressure allowed mathematically weaker individuals to survive longer. The survival of weaker individuals prevented the rapid domination of early local optima.
    \item \textbf{Enhanced Diversity Maintenance:} A fitness sharing mechanism utilising an expanded niche radius of 12 was 
    implemented. The expanded radius aggressively penalised candidate configurations possessing high spatial similarity. The spatial penalty forced the population to distribute across multiple distinct peaks in the search landscape rather than converging on a single topography.
    \item \textbf{High-Frequency Mutation:} The probability of mutation was increased 
    from 0.15 to 0.25. The elevated rate implied that on average four of the 16 active 
    antenna positions were perturbed in every offspring, constituting a 25\% structural 
    change per generation. The increased mutation rate tested the ability of the 
    algorithm to escape deep local optima through continuous, aggressive perturbation.
\end{enumerate}
...
The empirical study investigated the relative performance of population-based and trajectory-based 
metaheuristics applied to a highly deceptive search landscape. The benchmark problem utilised for 
evaluation was the MeerKAT 64-antenna array discrete subset selection problem. The physical coordinates 
of the 64 MeerKAT pads served as the static dataset.

The primary hypothesis postulated that the genetic algorithm variant two utilising enhanced diversity 
maintenance would achieve superior $uv$-coverage density compared to the standard 
evolutionary configurations and the simulated annealing baseline. The rugged nature of the spatial 
frequency landscape heavily penalises premature convergence. Therefore, explicit mathematical 
mechanisms designed to force population distribution across multiple local optima should theoretically 
yield superior global solutions. 
...
The four genetic algorithm configurations shared a common foundational structure. 
All variants utilised a static population size of 500 individuals. The evolutionary 
process executed for a maximum of 1000 generations per independent run. The multiplication 
of 500 individuals by 1000 generations yielded a maximum of 500,000 objective function 
evaluations per execution. An intersection crossover operator was applied unconditionally 
during parent recombination. Furthermore, a strict elitism mechanism preserved the single 
highest-scoring individual across every generation boundary.

The experimental variants modified specific control parameters to isolate the effects of 
selection pressure, diversity maintenance, and mutation frequency. The exact parameter 
configurations for the four evolutionary variants are summarised in Table~\ref{tab:ga_parameters}. 
...
\begin{table}[htbp]
    ...
    \resizebox{\columnwidth}{!}{
        \begin{tabular}{@{}lccc@{}}
            ...
            Variant 1 Reduced Selection & 2 & 0.15 & N/A \\
            Variant 2 Enhanced Diversity & 5 & 0.15 & 12 \\
            Variant 3 High Mutation & 3 & 0.25 & N/A \\
            \bottomrule
        \end{tabular}%
    }
\end{table}
...
baseline selection pressure. Tournament selection exerts pressure proportional to tournament size: a larger pool increases the probability 
that the highest-fitness individual is selected, thereby amplifying the reproductive advantage of superior configurations. A size of five was 
selected to provide directed exploitation without eliminating the stochastic variation required for continued exploration. Variant 1 reduced the tournament size to two. The reduction to two minimised selection pressure, 
...
Variant 2 implemented an expanded fitness sharing penalty utilising a niche radius of 12. A radius of 12 aggressively penalised 
structurally similar antenna layouts to enforce the primary hypothesis. Variant 3 substantially increased the mutation probability 
...
The expected behaviour of a trajectory-based
algorithm initialising from a single random 16-antenna layout is that the first evaluation reflects
...
well-calibrated geometric cooling schedule, where the temperature decreases sufficiently
...
Variant one reduced the tournament size from five to two. The reduction minimised selection pressure
...
but still substantially below the standard baseline GA.
 
Variant two implemented an expanded fitness sharing niche radius of 12 and exhibited
...
core generates an extensive volume of short baseline vectors that map directly to the central
...
volume of long baseline vectors that trace extensive elliptical arcs across the outer regions
...
optimum lies in a single clustered region diversity enforcement actively 
