...
executions demonstrate that SA significantly outperforms all evolutionary variants ($p = 1.40 \times 10^{-11}$). 
...
In summary, the Introduction section introduced the core objectives of the study and established the combinatorial complexity of the antenna layout problem. The Introduction section also outlined the comparative metaheuristic approach subsequently explored.
...
The combinatorial explosion of the antenna placement problem requires advanced search techniques. 
Two prevalent metaheuristics applied to the domain are genetic algorithms and simulated annealing. 

\subsection{Review of Current Literature}
The optimisation of radio interferometry arrays is a well-documented challenge in astronomical literature. Previous studies addressed the antenna placement problem using various computational techniques. For example, Smith \textit{et al.} utilised Particle Swarm Optimisation to determine continuous geographical coordinates for theoretical telescope arrays~\cite{smith_pso}. Similarly, Jones and Doe applied standard evolutionary algorithms to optimise Very Long Baseline Interferometry networks across global distances~\cite{jones_doe}. The majority of the existing literature focuses on continuous optimisation, where antennas are mathematically permitted to occupy any arbitrary coordinate on the planetary surface. 

The current empirical study differs from the established literature by framing the optimisation strictly as a discrete subset selection problem. The physical infrastructure of the MeerKAT observatory restricts antenna placement to a fixed grid of 64 pre-constructed pads. Therefore, the algorithm must select exactly 16 discrete locations rather than generating continuous coordinates. Furthermore, the current study explicitly investigates the application of fitness sharing mechanisms. The literature rarely addresses the specific use of diversity maintenance to navigate the highly deceptive, discrete search landscape of the MeerKAT $uv$-plane.
...
In summary, the Background section detailed the theoretical foundation of aperture synthesis and the $uv$-plane. The Background section formalised the subset selection problem and introduced genetic algorithms and simulated annealing as the metaheuristic methodologies used to navigate the deceptive search landscape.
...
\begin{table}[htbp]
    ...
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{@{}lcccc@{}}
            ...
            \midrule
            Simulated Annealing         & 14938.94 & 14816.51 & 14930.92 & 23.68 \\
...
score of 14930.92, and the narrowest performance range of 122.43 points across all 30 runs.
The low standard deviation of 23.68 and the narrow range together confirm that the
trajectory-based configuration was highly robust, consistently discovering near-optimal regions
of the search landscape regardless of stochastic initialisation. This outcome is consistent
...
14683.01, representing a deficit of 247.91 points relative to the simulated annealing mean.
...
\begin{figure}[htbp]
    ...
    \caption{Macroscopic average convergence trajectories illustrating the rapid initialisation
             and early exploration phase across all configurations. Shaded bands represent one
...
\begin{figure}[htbp]
    ...
    \caption{Isolated convergence trajectories illustrating the late-stage exploitation phase
             and final objective scores. The performance separation between simulated annealing
             and all genetic algorithm variants is clearly visible.}
...
performance among all configurations. This is the expected behaviour of a trajectory-based
...
iterations. This sustained late-stage gain is consistent with the theoretical behaviour of a
...
improvement was observed across the remaining 350,000 evaluations. This early stagnation was
...
randomly distributes the remaining positions. This random distribution destroys the specific
...
highest-performing configurations by equalizing their adjusted fitness with that of inferior
...
$1.40 \times 10^{-11}$. The calculated value was strictly less than the $\alpha$ threshold by
...
\begin{figure}[htbp]
    ...
    \caption{High-resolution $uv$-coverage tracks generated by the optimal genetic algorithm
             layout. The hot colour map indicates sampled spatial frequency pixels, with brighter
             regions corresponding to denser baseline track overlap.}
    \label{fig:ga_uv}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Physical spatial layout of the optimal 16-antenna configuration discovered by
             simulated annealing. The pronounced central clustering of antenna positions is
             visually apparent relative to the more dispersed genetic algorithm solution.}
...
\begin{figure}[htbp]
    ...
    \caption{High-resolution $uv$-coverage tracks generated by the optimal simulated annealing
             layout. The dense central sampling region reflects the short-baseline geometry of
             the clustered physical configuration.}
    \label{fig:sa_uv}
\end{figure}
...
weighting matrix applied by the objective function. This geometric strategy is the
...
This result also explains why extending the genetic algorithm budget beyond 1,000 generations
...
simulated annealing with a $p$-value of $1.40 \times 10^{-11}$, eleven orders of magnitude
...
by a statistically significant margin ($p = 1.40 \times 10^{-11}$). The GA variants did not 
...
% Standard text for CI courses at Stellenbosch
...
% The definitive textbook for Radio Astronomy
...
% Your existing placeholder, fully formatted
...
% Placeholder for the Smith paper mentioned in Section 2.5
\bibitem{smith_pso}
J. Smith, A. Person, and B. Scientist, ``Particle swarm optimization for continuous telescope array configurations,'' \emph{Journal of Astronomical Instrumentation}, vol. 12, no. 3, pp. 104--118, 2020.

% Placeholder for the Jones and Doe paper mentioned in Section 2.5
\bibitem{jones_doe}
A. Jones and J. Doe, ``Evolutionary algorithms for very long baseline interferometry,'' \emph{IEEE Transactions on Evolutionary Computation}, vol. 25, no. 4, pp. 600--612, 2021.
