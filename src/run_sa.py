import numpy as np
import physics 
from alg_solvers import SimulatedAnnealing
import time
...
if __name__ == "__main__":
    NUM_RUNS = 30
    TOTAL_EVALUATIONS = 500000  # 500 pop * 1000 gens
    
    all_histories = []
    best_overall_score = -float('inf')
    best_overall_layout = None

    print(f"--- Starting Simulated Annealing: {NUM_RUNS} Runs ---")
    start_time = time.time()

    for run in range(NUM_RUNS):
        print(f"\nStarting Run {run + 1}/{NUM_RUNS}...")
        sa = SimulatedAnnealing(num_antennas=64, k=16, max_iterations=TOTAL_EVALUATIONS, cooling_rate=0.99999)
        best_layout, best_score, history = sa.run(fitness_function=evaluate_layout)
    ...
    print(f"\nAll {NUM_RUNS} SA Runs Completed in {total_time/60:.2f} minutes.")
    print(f"Absolute Best Score Found: {best_overall_score}")
    
    # Save the 2D array of histories (30 runs, 100000 iterations)
    np.save("../out/sa_all_histories.npy", np.array(all_histories))
    np.save("../out/sa_absolute_best_layout.npy", best_overall_layout)
    print("Data saved to out/sa_all_histories.npy and out/sa_absolute_best_layout.npy")